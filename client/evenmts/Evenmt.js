...
import DeleteEvenmt from './DeleteEvenmt'
...
export default function Evenmt ({match}) {
  const classes = useStyles()
  const [stats, setStats] = useState({})
  const [evenmt, setEvenmt] = useState({manager:{}})
  ...
  useEffect(() => {
    ...
    enrollmentStats({evenmtId: match.params.evenmtId}, {t:jwt.token}, signal).then((data) => {
      if (data.error) {
        setValues({...values, error: data.error})
      } else {
        setStats(data)
  ...
  const removeEvenmt = (evenmt) => {
    setValues({...values, redirect:true})
  }
  const addActivity = (evenmt) => {
    setEvenmt(evenmt)
  }
  const clickPublish = () => {
    if(evenmt.activities.length > 0){    
      setOpen(true)
    }
  }
  const publish = () => {
    let EvenmtData = new FormData()
    EvenmtData.append('published', true)
      update({
        evenmtId: match.params.evenmtId
          ...
          if (data && data.error) {
            ...
            setEvenmt({...evenmt, published: true})
            setOpen(false)
          }
      })
  }
  ...
  if (values.redirect) {
    return (<Redirect to={'/teach/evenmts'}/>)
  }
              ...
              <Card className={classes.card}>
                <CardHeader
                  ...
                  subheader={<div>
                        ...
                        <span className={classes.category}>{evenmt.category}</span>
                      </div>
                    }
                  action={<>
             {auth.isAuthenticated().user && auth.isAuthenticated().user._id == evenmt.manager._id &&
                (<span className={classes.action}>
                  <Link to={"/teach/evenmt/edit/" + evenmt._id}>
                    <IconButton aria-label="Edit" color="secondary">
                      <Edit/>
                    </IconButton>
                  </Link>
                {!evenmt.published ? (<>
                  <Button color="secondary" variant="outlined" onClick={clickPublish}>{evenmt.activities.length == 0 ? "Add atleast 1 activity to publish" : "Publish"}</Button>
                  <DeleteEvenmt evenmt={evenmt} onRemove={removeEvenmt}/>
                </>) : (
                  <Button color="primary" variant="outlined">Published</Button>
                )}
                </span>)
             }
                {evenmt.published && (<div>
                ...
                <div className={classes.flex}>
                  ...
                  <div className={classes.details}>
                    <Typography variant="body1" className={classes.subheading}>
                        {evenmt.description}<br/>
                    </Typography>
                    
              {evenmt.published && <div className={classes.enroll}><Enroll evenmtId={evenmt._id}/></div>} 
                ...
                <CardHeader
                  ...
                  action={
             auth.isAuthenticated().user && auth.isAuthenticated().user._id == evenmt.manager._id && !evenmt.published &&
                (<span className={classes.action}>
                  <NewActivity evenmtId={evenmt._id} addActivity={addActivity}/>
                </span>)
            }
                />
                <List>
              ...
              <Dialog open={open} onClose={handleClose} aria-labelledby="form-dialog-title">
                ...
                <DialogContent>
                  <Typography variant="body1">Publishing your event will make it live to employees for enrollment. </Typography><Typography variant="body1">Make sure all activities are added and ready for publishing.</Typography></DialogContent>
                <DialogActions>
