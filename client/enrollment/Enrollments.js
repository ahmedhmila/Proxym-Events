    ...
    return (
      <div>
        <GridList cellHeight={120} className={classes.gridList} cols={4}>
          {props.enrollments.map((evenmt, i) => (
            <GridListTile key={i} className={classes.tile}>
              <Link to={"/learn/"+evenmt._id}><img className={classes.image} src={'/api/evenmts/photo/'+evenmt.evenmt._id} alt={evenmt.evenmt.name} /></Link>
              <GridListTileBar className={classes.tileBar}
                title={<Link to={"/learn/"+evenmt._id} className={classes.tileTitle}>{evenmt.evenmt.name}</Link>}
                actionIcon={<div className={classes.action}>
                 {evenmt.completed ? (<CompletedIcon color="secondary"/>)
